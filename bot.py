...
# PROTECT + MENU + CONTATTI + VETRINA
# /status /utenti /backup /restore_db (MERGE)
# /broadcast + /broadcast_delete (ultimo broadcast finché non riavvii)
# NO PIN (così i bottoni restano sempre visibili)
# =====================================================
...
PHOTO_URL = os.environ.get("PHOTO_URL", "METTI_LA_TUA_PHOTO_URL")
...
def is_admin(uid: int | None) -> bool:
    if not ADMIN_IDS:
        return True  # se non imposti ADMIN_IDS, sei admin di default
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Foto + messaggio con bottoni (sempre visibili)
    try:
...
async def broadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if m.reply_to_message:
        ...
        if not text_body:
            await m.reply_text("Uso: /broadcast <testo> oppure rispondi a un contenuto e fai /broadcast", protect_content=True)
            return
